...
graph:
	...
	@cp -f cmd/weaving_net/main.go .
	go-callvis -skipbrowser -format=svg -nostd -file=weaving_net weaving_net
	@rm -f main.go weaving_net.gv
...
.PHONY: build
# build weaving_net for linux amd64 binary
build:
	@echo "building 'weaving_net', linux binary file will output to 'cmd/weaving_net'"
	@cd cmd/weaving_net && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build
