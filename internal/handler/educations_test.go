...
func newEducationsHandler() *gotest.Handler {
	...
	d.IDao = dao.NewEducationsDao(d.DB, c.ICache.(cache.EducationsCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &educationsHandler{iDao: d.IDao.(dao.EducationsDao)}
	iHandler := h.IHandler.(EducationsHandler)
	...
	testFns := []gotest.RouterInfo{
		...
		},
		{
			...
			Method:      http.MethodPost,
			Path:        "/educations/condition",
			HandlerFunc: iHandler.GetByCondition,
		},
		{
			FuncName:    "ListByIDs",
			Method:      http.MethodPost,
			Path:        "/educations/list/ids",
...
func Test_educationsHandler_Create(t *testing.T) {
	h := newEducationsHandler()
	defer h.Close()
	testData := &types.CreateEducationsRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Educations))
...
func Test_educationsHandler_DeleteByID(t *testing.T) {
	h := newEducationsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Educations)
...
func Test_educationsHandler_DeleteByIDs(t *testing.T) {
	h := newEducationsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Educations)
	...
	result := &gohttp.StdResult{}
	err := gohttp.Post(result, h.GetRequestURL("DeleteByIDs"), &types.DeleteEducationssByIDsRequest{IDs: []uint64{testData.ID}})
	if err != nil {
		t.Fatal(err)
	}
	if result.Code != 0 {
	...
	// get error test
	err = gohttp.Post(result, h.GetRequestURL("DeleteByIDs"), &types.DeleteEducationssByIDsRequest{IDs: []uint64{111}})
	assert.Error(t, err)
}

func Test_educationsHandler_UpdateByID(t *testing.T) {
	h := newEducationsHandler()
	defer h.Close()
	testData := &types.UpdateEducationsByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Educations))
...
func Test_educationsHandler_GetByID(t *testing.T) {
	h := newEducationsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Educations)
...
func Test_educationsHandler_GetByCondition(t *testing.T) {
	h := newEducationsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Educations)
	...
	result := &gohttp.StdResult{}
	err := gohttp.Post(result, h.GetRequestURL("GetByCondition"), &types.GetEducationsByConditionRequest{
		query.Conditions{
...
func Test_educationsHandler_ListByIDs(t *testing.T) {
	h := newEducationsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Educations)
	...
	result := &gohttp.StdResult{}
	err := gohttp.Post(result, h.GetRequestURL("ListByIDs"), &types.ListEducationssByIDsRequest{IDs: []uint64{testData.ID}})
	if err != nil {
		t.Fatal(err)
	}
	if result.Code != 0 {
	...
	// get error test
	err = gohttp.Post(result, h.GetRequestURL("ListByIDs"), &types.ListEducationssByIDsRequest{IDs: []uint64{111}})
	assert.Error(t, err)
}

func Test_educationsHandler_ListByLastID(t *testing.T) {
	h := newEducationsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Educations)
...
func Test_educationsHandler_List(t *testing.T) {
	h := newEducationsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Educations)
	...
	result := &gohttp.StdResult{}
	err := gohttp.Post(result, h.GetRequestURL("List"), &types.ListEducationssRequest{query.Params{
		Page: 0,
		Size: 10,
...
func TestNewEducationsHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewEducationsHandler()
}
