...
func newProjectsHandler() *gotest.Handler {
	...
	d.IDao = dao.NewProjectsDao(d.DB, c.ICache.(cache.ProjectsCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &projectsHandler{iDao: d.IDao.(dao.ProjectsDao)}
	iHandler := h.IHandler.(ProjectsHandler)
...
func Test_projectsHandler_Create(t *testing.T) {
	h := newProjectsHandler()
	defer h.Close()
	testData := &types.CreateProjectsRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Projects))
...
func Test_projectsHandler_DeleteByID(t *testing.T) {
	h := newProjectsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Projects)
...
func Test_projectsHandler_DeleteByIDs(t *testing.T) {
	h := newProjectsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Projects)
	...
	result := &gohttp.StdResult{}
	err := gohttp.Post(result, h.GetRequestURL("DeleteByIDs"), &types.DeleteProjectssByIDsRequest{IDs: []uint64{testData.ID}})
	if err != nil {
		t.Fatal(err)
	}
	if result.Code != 0 {
	...
	// get error test
	err = gohttp.Post(result, h.GetRequestURL("DeleteByIDs"), &types.DeleteProjectssByIDsRequest{IDs: []uint64{111}})
	assert.Error(t, err)
}

func Test_projectsHandler_UpdateByID(t *testing.T) {
	h := newProjectsHandler()
	defer h.Close()
	testData := &types.UpdateProjectsByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Projects))
...
func Test_projectsHandler_GetByID(t *testing.T) {
	h := newProjectsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Projects)
...
func Test_projectsHandler_GetByCondition(t *testing.T) {
	h := newProjectsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Projects)
	...
	result := &gohttp.StdResult{}
	err := gohttp.Post(result, h.GetRequestURL("GetByCondition"), &types.GetProjectsByConditionRequest{
		query.Conditions{
...
func Test_projectsHandler_ListByIDs(t *testing.T) {
	h := newProjectsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Projects)
	...
	result := &gohttp.StdResult{}
	err := gohttp.Post(result, h.GetRequestURL("ListByIDs"), &types.ListProjectssByIDsRequest{IDs: []uint64{testData.ID}})
	if err != nil {
		t.Fatal(err)
	}
	if result.Code != 0 {
	...
	// get error test
	err = gohttp.Post(result, h.GetRequestURL("ListByIDs"), &types.ListProjectssByIDsRequest{IDs: []uint64{111}})
	assert.Error(t, err)
}

func Test_projectsHandler_ListByLastID(t *testing.T) {
	h := newProjectsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Projects)
...
func Test_projectsHandler_List(t *testing.T) {
	h := newProjectsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Projects)
	...
	result := &gohttp.StdResult{}
	err := gohttp.Post(result, h.GetRequestURL("List"), &types.ListProjectssRequest{query.Params{
		Page: 0,
		Size: 10,
