...
func newSkillsHandler() *gotest.Handler {
	...
	d.IDao = dao.NewSkillsDao(d.DB, c.ICache.(cache.SkillsCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &skillsHandler{iDao: d.IDao.(dao.SkillsDao)}
	iHandler := h.IHandler.(SkillsHandler)
	...
	testFns := []gotest.RouterInfo{
		...
		},
		{
			...
			Method:      http.MethodPost,
			Path:        "/skills/condition",
			HandlerFunc: iHandler.GetByCondition,
		},
		{
			FuncName:    "ListByIDs",
			Method:      http.MethodPost,
			Path:        "/skills/list/ids",
...
func Test_skillsHandler_Create(t *testing.T) {
	h := newSkillsHandler()
	defer h.Close()
	testData := &types.CreateSkillsRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Skills))
...
func Test_skillsHandler_DeleteByID(t *testing.T) {
	h := newSkillsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Skills)
...
func Test_skillsHandler_DeleteByIDs(t *testing.T) {
	h := newSkillsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Skills)
	...
	result := &gohttp.StdResult{}
	err := gohttp.Post(result, h.GetRequestURL("DeleteByIDs"), &types.DeleteSkillssByIDsRequest{IDs: []uint64{testData.ID}})
	if err != nil {
		t.Fatal(err)
	}
	if result.Code != 0 {
	...
	// get error test
	err = gohttp.Post(result, h.GetRequestURL("DeleteByIDs"), &types.DeleteSkillssByIDsRequest{IDs: []uint64{111}})
	assert.Error(t, err)
}

func Test_skillsHandler_UpdateByID(t *testing.T) {
	h := newSkillsHandler()
	defer h.Close()
	testData := &types.UpdateSkillsByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Skills))
...
func Test_skillsHandler_GetByID(t *testing.T) {
	h := newSkillsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Skills)
...
func Test_skillsHandler_GetByCondition(t *testing.T) {
	h := newSkillsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Skills)
	...
	result := &gohttp.StdResult{}
	err := gohttp.Post(result, h.GetRequestURL("GetByCondition"), &types.GetSkillsByConditionRequest{
		query.Conditions{
...
func Test_skillsHandler_ListByIDs(t *testing.T) {
	h := newSkillsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Skills)
	...
	result := &gohttp.StdResult{}
	err := gohttp.Post(result, h.GetRequestURL("ListByIDs"), &types.ListSkillssByIDsRequest{IDs: []uint64{testData.ID}})
	if err != nil {
		t.Fatal(err)
	}
	if result.Code != 0 {
	...
	// get error test
	err = gohttp.Post(result, h.GetRequestURL("ListByIDs"), &types.ListSkillssByIDsRequest{IDs: []uint64{111}})
	assert.Error(t, err)
}

func Test_skillsHandler_ListByLastID(t *testing.T) {
	h := newSkillsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Skills)
...
func Test_skillsHandler_List(t *testing.T) {
	h := newSkillsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Skills)
	...
	result := &gohttp.StdResult{}
	err := gohttp.Post(result, h.GetRequestURL("List"), &types.ListSkillssRequest{query.Params{
		Page: 0,
		Size: 10,
